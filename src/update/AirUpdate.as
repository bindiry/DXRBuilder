package update
{
	...
	public class AirUpdate extends EventDispatcher
	{
		...
		public function AirUpdate(ui:IAirUpdateUI = null,configPath:String = null)
		{
			this.ui = ui;
			if(!configPath){
				localConfigPath = File.applicationDirectory.nativePath+"/update.xml";
			}else{
		...
		private function configloadComplete(byte:ByteArray):void{
			...
			try{
				...
				if(getCurrentAppVersion() != serverupdatevo.version){
					//检测到2个版本不一样.开始请求更新
					...
					//如果版本相同 把下载下来的config写入本地
					byte.position = 0;
					OpenFile.write(byte,localConfigPath);
				}
			}catch(e:Error){
				error_action();
				return;
